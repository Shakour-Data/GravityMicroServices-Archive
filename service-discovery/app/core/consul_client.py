...
class ConsulClient:
    ...
    def __init__(self):
        ...
        logger.info(
            ...
            f"{settings.CONSUL_HOST}:{settings.CONSUL_PORT}"
        )
    ...
    async def update_health_check(self, check_id: str, status: str, output: str = "") -> bool:
        ...
        try:
            if status == "pass":
                success = self.client.agent.check.ttl_pass(check_id, output)
            elif status == "warn":
                success = self.client.agent.check.ttl_warn(check_id, output)
            elif status == "fail":
                success = self.client.agent.check.ttl_fail(check_id, output)
            else:
